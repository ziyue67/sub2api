...
func TestGetTokenLeaderboardWithFiltersUsesGenericAccountNameSearch(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	queryPattern := `(?s)LEFT JOIN accounts a ON u\.account_id = a\.id.*LEFT JOIN accounts parent_a ON a\.parent_account_id = parent_a\.id.*\(a\.name ILIKE \$3 OR parent_a\.name ILIKE \$3\).*LIMIT \$4`
	mock.ExpectQuery(queryPattern).
		WithArgs(start, end, "%analytics-main%", 20).
		WillReturnRows(sqlmock.NewRows([]string{
			"user_id", "email", "requests", "total_tokens", "input_tokens", "output_tokens",
			"cache_tokens", "image_output_tokens", "cost", "actual_cost", "account_cost", "last_active_at",
		}))
...
func TestGetTokenLeaderboardWithFiltersUsesGenericAccountEmailSearch(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	queryPattern := `(?s)LEFT JOIN accounts a ON u\.account_id = a\.id.*LEFT JOIN accounts parent_a ON a\.parent_account_id = parent_a\.id.*a\.extra->>'email_address' ILIKE \$3.*a\.extra->>'email' ILIKE \$3.*a\.credentials->>'email' ILIKE \$3.*parent_a\.extra->>'email_address' ILIKE \$3.*parent_a\.extra->>'email' ILIKE \$3.*parent_a\.credentials->>'email' ILIKE \$3.*LIMIT \$4`
	mock.ExpectQuery(queryPattern).
		WithArgs(start, end, "%owner@example.net%", 20).
		WillReturnRows(sqlmock.NewRows([]string{
			"user_id", "email", "requests", "total_tokens", "input_tokens", "output_tokens",
			"cache_tokens", "image_output_tokens", "cost", "actual_cost", "account_cost", "last_active_at",
		}))
	...
	rows, err := repo.GetTokenLeaderboardWithFilters(context.Background(), start, end, 20, usagestats.TokenLeaderboardQuery{
		AccountEmail: "owner@example.net",
	})
...
func TestGetTokenLeaderboardWithFiltersCombinesAccountNameAndEmailSearch(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	start := time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)

	queryPattern := `(?s)\(a\.name ILIKE \$3 OR parent_a\.name ILIKE \$3\).*a\.extra->>'email_address' ILIKE \$4.*a\.extra->>'email' ILIKE \$4.*a\.credentials->>'email' ILIKE \$4.*parent_a\.extra->>'email_address' ILIKE \$4.*parent_a\.extra->>'email' ILIKE \$4.*parent_a\.credentials->>'email' ILIKE \$4.*LIMIT \$5`
	mock.ExpectQuery(queryPattern).
		WithArgs(start, end, "%analytics-main%", "%owner@example.net%", 20).
		WillReturnRows(sqlmock.NewRows([]string{
			"user_id", "email", "requests", "total_tokens", "input_tokens", "output_tokens",
			"cache_tokens", "image_output_tokens", "cost", "actual_cost", "account_cost", "last_active_at",
		}))
