...
func (a *Account) GetOpenAIBaseURL() string {
	...
	// Fork(#46)：OpenAI 兼容自定义端点允许 OpenAI APIKey 账号走分协议地址（api_base_urls）。
	// 上游：多协议 APIKey 供应商（国产 + OpenCode Go）。两者取并集，缺一不可。
	if (a.IsMultiProtocolAPIKey() || a.IsOpenAIApiKey()) && a.IsAdaptiveAPIProtocol() {
...
func (a *Account) GetAPIProtocol() string {
	// Fork(#46)：OpenAI APIKey 亦支持分协议端点；上游：多协议 APIKey 供应商（国产 + OpenCode Go）。
	if a == nil || (!a.IsMultiProtocolAPIKey() && !a.IsOpenAIApiKey()) {
...
func (a *Account) GetAnthropicProtocolBaseURL() string {
	...
	// 上游：多协议 APIKey 供应商（国产 + OpenCode Go）共用同一条分协议 base 路径；
	// Fork：OpenAI APIKey 已在上面单独分流，不受此守卫影响。
	if !a.IsMultiProtocolAPIKey() || (!a.IsAnthropicProtocol() && !a.IsAdaptiveAPIProtocol()) {
		return ""
	}
	if a.IsAdaptiveAPIProtocol() {
