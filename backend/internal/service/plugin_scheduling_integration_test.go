...
func TestCPAAdvancedCoreProcessIntegration(t *testing.T) {
	...
	require.NoError(t, err)
	// 断言"GetInfo 与清单一致"这个不变量，而非某个写死的 ID：
	// 基础变体（-id-suffix）会改 ID，写死会让测试只对完整变体有效。
	require.Equal(t, readManifestFieldForTest(t, raw, "id"), info.GetPluginId(),
		"GetInfo.plugin_id 必须与包内清单一致")
	require.Equal(t, readManifestFieldForTest(t, raw, "version"), info.GetPluginVersion(),
		"GetInfo.plugin_version 必须与包内清单一致")
