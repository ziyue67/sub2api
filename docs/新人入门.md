...
本 Fork **只分发 Docker 镜像**（`linux/amd64`），GitHub Releases 不再提供 `tar.gz` / `zip` 二进制包，
安装脚本改为 Docker Compose 部署：自动下载 compose 文件、生成 `.env` 密钥、固定镜像 tag 并启动容器。

在 VPS 的 Linux 终端执行：

```bash
# 安装指定版本（会启动 sub2api + PostgreSQL + Redis）
curl -fsSL https://raw.githubusercontent.com/ziyue67/sub2api/v0.2.81/deploy/install.sh \
  | sudo bash -s -- install -v v0.2.81
```

脚本默认安装到 `/opt/sub2api`，常用操作：
...
# 回滚 / 切换版本
curl -fsSL https://raw.githubusercontent.com/ziyue67/sub2api/v0.2.81/deploy/install.sh \
  | sudo bash -s -- rollback v0.2.81

# 查看容器状态与日志
cd /opt/sub2api && docker compose ps && docker compose logs -f sub2api
```
...
如果 VPS 上还跑着旧版的二进制部署（systemd + `/opt/sub2api/sub2api`），请先停用旧服务并备份数据库，再按上面的 Docker 流程部署；
不要同时启动两个占用同一端口的版本，也不要删除原数据目录。升级后在后台确认版本为 `v0.2.81`，并检查账号、分组、渠道和历史数据。
