...
在原有采集设置中选择 SSE 或 WebSocket、目标网关（默认 `unified-88`；已保存的其他网关设置保持不变），以及可选的公网边缘 IP。边缘 IP 留空时使用 DNS。仍使用原有采票代理、范围、请求预算和取消机制。WebSocket 下游若由 HTTP bridge 转为 SSE，则选择 SSE 采票。
...
- 路由 pair 独立缓存，最多 256 项，只保留路由 Cookie 和凭据摘要键。同账号、同凭据、同目标策略及采票协议下可跨模型复用；模型声明不匹配不丢掉已验收的目标路由。按 JWT exp 过期，票仍逐模型验收、最多 240 秒。
- 定向请求可复用仍有效的原 pair。服务端只轮换一半或删除 Cookie 时拒收，不拼接不同轮次的 pair；被拒绝的缓存路由不会从旧票恢复。缓存不跨账号共享，重启后按现有票据恢复或重新获取。
...
参考：https://linux.do/t/topic/2947373 。将独立路由缓存和逐模型验收接入既有 Go 采集器，无需另建云函数。支持 `88`、`unified_88`、`unified.88` 等网关别名，保存时规范化为 `unified-88`。

HTTP 400/404/422 或 SSE/WS 中的明确请求、模型拒绝停止当前模型重试，允许继续其他模型；401/403 和认证错误停止该账号本轮采集。未知错误保留原有重试预算、超时、冷却及取消机制。错误详情仅保留受控代码，不记录上游消息正文。

不自动打开采票开关，不改变已保存的出口轮换规则。路由 Cookie 固定网关，不等于固定出口或能力验证。
