export default {
    settings: {
      ...
      billingRisk: {
        title: 'Balance Overspend Protection',
        description: 'Reserves estimated cost before forwarding so concurrent requests from the same user share the available balance. This reduces overspend from low-balance or high-cost requests. Saved values apply to new requests immediately.',
        enabled: 'Enable balance overspend protection',
        enabledHint: 'Disabled by default. When enabled, the gateway reserves funds and rejects new requests when the risk budget is insufficient. When disabled, new requests skip all risk checks; existing reservations settle or expire automatically.',
        budget: 'Protection budget',
        lifecycle: 'Reservation lifetime (seconds)',
        fields: {
          lowBalanceThreshold: 'Text protection starts at',
          lowBalanceThresholdHint: 'Text requests reserve funds when the balance is at or below this amount. Default: 10. Images, audio, and search also use estimated request cost; video is always protected.',
          safetyFactor: 'Estimated cost multiplier',
          safetyFactorHint: 'Adds headroom to cost estimates. The default 1.25 reserves 125% of the estimate; higher values are more conservative.',
          minimumRequestRisk: 'Minimum reserved per request',
          minimumRequestRiskHint: 'Very cheap requests still reserve at least this amount to limit unlimited low-cost concurrency. Default: 0.001.',
          overdraftAllowance: 'Extra risk allowance',
          overdraftAllowanceHint: 'The additional risk budget allowed when the balance is just short. Default: 0.2. Set to 0 for stricter enforcement.',
          highCostTrigger: 'High-cost media protection starts at',
          highCostTriggerHint: 'Above the text threshold, images, audio, and search are still protected when estimated cost reaches this amount. Default: 1. Video does not use this threshold.',
          leaseTTL: 'Standard reservation limit',
          leaseTTLHint: 'How long a standard request keeps its reservation without refresh or settlement. Default: 60 seconds. This is not the request timeout.',
          refreshInterval: 'Long-request refresh frequency',
          refreshIntervalHint: 'How often streaming or long-running requests extend their reservation. Default: 15 seconds. It must be less than half the standard limit; shorter values increase Redis traffic.',
          uncertainCooldown: 'Uncertain billing hold time',
          uncertainCooldownHint: 'Keeps funds reserved when billing cannot be confirmed, preventing the budget from reopening immediately. Default: 300 seconds.',
          videoLeaseTTL: 'Video reservation limit',
          videoLeaseTTLHint: 'Video jobs can run asynchronously, so they use a separate maximum reservation time. Default: 86400 seconds (24 hours).',
          idleBalanceTTL: 'User risk state retention',
          idleBalanceTTLHint: 'How long Redis keeps a user risk balance after no active reservations remain. Default: 120 seconds; it is rebuilt from the latest balance after expiry.',
        },
        errors: {
          safetyFactor: 'The estimated cost multiplier must be a finite number greater than or equal to 1.',
          amount: 'Risk amount settings must be finite non-negative numbers.',
          refreshInterval: 'The long-request refresh frequency must be a positive integer.',
          leaseTTL: 'The standard reservation limit must be greater than twice the refresh frequency.',
          duration: 'The uncertain billing hold time and user risk state retention must be positive integers.',
          videoTTL: 'The video reservation limit must be a positive integer no shorter than the standard reservation limit.',
        },
      },
      webSearchEmulation: {
