export default {
    settings: {
      ...
      billingRisk: {
        title: '余额超额消费保护',
        description: '在请求转发前预留预计费用，让同一用户的并发请求共享可用余额，降低低余额或高价请求同时放行造成的超额消费。保存后立即对新请求生效。',
        enabled: '启用余额超额消费保护',
        enabledHint: '默认关闭。开启后会正式预留额度，并在可用风险额度不足时拒绝新请求；关闭后新请求完全不做风险检查，已有占额会随结算或到期自动清理。',
        budget: '保护额度参数',
        lifecycle: '占额有效时间（秒）',
        fields: {
          lowBalanceThreshold: '文本请求保护起点（元）',
          lowBalanceThresholdHint: '余额不高于该金额时，文本请求也会预留费用。默认 10 元；图片、音频、搜索等还会按单次预计费用判断，视频始终保护。',
          safetyFactor: '预留费用放大倍数',
          safetyFactorHint: '用来给费用估算留出余量。默认 1.25，表示按预计费用的 125% 预留；数值越大越保守。',
          minimumRequestRisk: '单个请求最低预留（元）',
          minimumRequestRiskHint: '预计费用很低时也至少预留这笔金额，避免大量低价请求无限并发。默认 0.001 元。',
          overdraftAllowance: '额外风险缓冲（元）',
          overdraftAllowanceHint: '余额刚好不足时，最多额外允许占用的风险额度。默认 0.2 元；设为 0 会更严格。',
          highCostTrigger: '高价媒体保护起点（元）',
          highCostTriggerHint: '余额高于文本保护起点时，图片、音频、搜索等单次预计费用达到该金额仍会强制保护。默认 1 元，视频不受此值限制。',
          leaseTTL: '普通请求占额上限',
          leaseTTLHint: '普通请求没有继续续期或结算时，最多保留额度多久。默认 60 秒；这不是请求超时时间。',
          refreshInterval: '长请求续期频率',
          refreshIntervalHint: '流式或耗时请求每隔多久延长一次占额。默认 15 秒；必须小于普通占额上限的一半，设置过短会增加 Redis 请求。',
          uncertainCooldown: '扣费异常保留时间',
          uncertainCooldownHint: '扣费结果暂时无法确认时，继续保留额度多久，避免立刻放开预算造成重复消费。默认 300 秒。',
          videoLeaseTTL: '视频任务占额上限',
          videoLeaseTTLHint: '视频任务可能异步执行较久，因此使用单独的最长占额时间。默认 86400 秒（24 小时）。',
          idleBalanceTTL: '用户风险状态保留时间',
          idleBalanceTTLHint: '用户没有活跃占额后，Redis 继续保留其风险余额状态多久。默认 120 秒，过期后会按最新余额重新建立。',
        },
        errors: {
          safetyFactor: '预留费用放大倍数必须是不小于 1 的有限数值。',
          amount: '风险金额参数必须是非负有限数值。',
          refreshInterval: '长请求续期频率必须是正整数。',
          leaseTTL: '普通请求占额上限必须大于两倍续期频率。',
          duration: '扣费异常保留时间和用户风险状态保留时间必须是正整数。',
          videoTTL: '视频任务占额上限必须是不短于普通请求占额上限的正整数。',
        },
      },
      webSearchEmulation: {
        title: 'Web Search 模拟',
